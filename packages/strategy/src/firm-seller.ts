...
interface FirmSellerConfig {
  concessionRate?: number  // fraction of range conceded per round (default 0.05)
}

export class FirmSeller implements SellerStrategy {
  private concessionRate: number

  constructor(config?: FirmSellerConfig) {
    this.concessionRate = config?.concessionRate ?? 0.05
  }
